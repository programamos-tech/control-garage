      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 space-y-4">
          {dict.faq.items.map((item) => (
            <details
              key={item.q}
              className="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm open:ring-2 open:ring-brand-gold/30"
            >
              <summary className="cursor-pointer list-none font-bold text-brand-blue">
                <span className="flex items-center justify-between gap-2">
                  {item.q}
                  <span className="text-brand-gold-dark transition group-open:rotate-180">▼</span>
                </span>
              </summary>
